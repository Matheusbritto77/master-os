...
(function() {
    ...
    // Redireciona a página se estiver em HTTP
    if (location.protocol === 'http:') {
        location.href = location.href.replace('http:', 'https:');
    }

    // Override para funções JS globais que usam URLs
    const overrideFunction = (fnName) => {
        const originalFn = window[fnName];
        if (typeof originalFn === 'function') {
            window[fnName] = function(...args) {
                const httpsArgs = args.map(arg =>
                    typeof arg === 'string' && arg.startsWith('http:') ? toHTTPS(arg) : arg
                );
                return originalFn.apply(this, httpsArgs);
            };
            console.log(`Função interceptada: ${fnName}`);
        }
    };

    // Adapte aqui os nomes das funções globais do seu projeto que recebem URLs
    const functionsToIntercept = ['abrirModal', 'abrirPDF', 'baixarArquivo', 'verCertificado'];
    functionsToIntercept.forEach(overrideFunction);

    // Interceptar cliques diretos em links com href "http:"
    document.addEventListener('click', (e) => {
        const el = e.target.closest('a[href^="http:"]');
        if (el) {
            e.preventDefault();
            const httpsURL = toHTTPS(el.href);
            window.location.href = httpsURL;
        }
    });

    // Interceptar formulários com action "http:"
    document.addEventListener('submit', (e) => {
        const form = e.target;
        if (form.action && form.action.startsWith('http:')) {
            form.action = toHTTPS(form.action);
        }
    });

    // Corrigir atributos inline assim que o DOM carregar
    function fixInlineAttributes() {
        const tags = document.querySelectorAll('[href^="http:"], [src^="http:"], [action^="http:"], [style*="http:"]');
        tags.forEach(el => {
            ['href', 'src', 'action'].forEach(attr => {
                if (el.getAttribute(attr)?.startsWith('http:')) {
                    el.setAttribute(attr, toHTTPS(el.getAttribute(attr)));
                }
            });
            if (el.hasAttribute('style')) {
                el.setAttribute('style', el.getAttribute('style').replace(/http:/g, 'https:'));
            }
        });
    }

    // Reobserva DOM dinâmico
    if (window.MutationObserver) {
        new MutationObserver(() => fixInlineAttributes())
            .observe(document.body, { childList: true, subtree: true });
    }

    // Executar assim que possível
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', fixInlineAttributes);
    } else {
        fixInlineAttributes();
    }

    console.log('HTTPS interceptor de funções JS e links HTML ativo.');
})();
</script>
